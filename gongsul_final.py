...
ASSETS_PATH = OUTPUT_PATH / Path(r"Lost 112\build\assets\frame0") #사진 모아놓은 폴더 경로
...
ATC_ID=[] #메일 중복 방지하기 위해 이미 보낸 습득물 관리번호 저장하는 리스트
...
def search_data(category_value,convert_value, user_email, keyword):
    global idxxx
    global Start_Ymd
    global End_Ymd
    global Id
    ...
    def while_hamsu():
        ...
        keyword_search=re.compile(keyword).search(arr[0])
       
        
        #키워드 입력 안 했을 경우
        if keyword==NONE or "키워드 입력":
            if Id in ATC_ID:  #리스트에 관리번호 있으면 다음으로
                pass
        ...
        #키워드 입력 했을 경우                
            ...
            if keyword_search!=None:  #키워드가 일치할 때
                if Id in ATC_ID: 
                    pass
                else:
                    mail(user_email, arr)
...
def find_screen():
    ...
    search_data(category_value,convert_value, user_email, keyword)
#=======================================================================
# 새로운 창 띄우기
#=======================================================================
def start_find():
...
canvas.create_text(
    ...
    475.0,
    anchor="nw",
    text="※키워드는 간단하게 적어주세요. \nex)아이폰 시리즈(SE, 11 등) -> 아이폰",
    fill="#FFFFFF",
    font=("맑은 고딕", 15 * -1)
)
...
def createCategoryWindow():
    ...
    def on_enter_keyword(e):
        keyword_blank=keyword_entry.get()
        if keyword_blank=="키워드 입력":
            keyword_entry.delete(0,"end")
            
    def on_leave_keyword(e):
        keyword_blank=keyword_entry.get()
        if keyword_blank=="":
            keyword_entry.insert(0,"키워드 입력")
    keyword_entry = Entry(category) 
    keyword_entry.place(
        width=100,
        x=496.0,
        y=430.0,
    ...
    keyword_entry.insert(0, "키워드 입력")
    keyword_entry.bind("<FocusIn>",on_enter_keyword)
    keyword_entry.bind("<FocusOut>",on_leave_keyword)
    #===============
    #==============
    def btn_3_cmd():    
